...
Algumas das análises mais interessantes são aquelas específicas a um contexto. Por exemplo: se uma palavra é um verbo ou
se uma palavra é o nome de uma pessoa.
...
spacy download. Por exemplo, o pacote "en_core_web_sm" é um fluxo de processamento pequeno em inglês que foi treinado 
com texto da internet e possui diversos recursos.
...
configuração utilizado para treiná-lo. Ele informa qual o idioma a ser utilizado e como configurar o fluxo de 
...
Em adição à previsão de classes gramaticais, podemos prever como as palavras estão 
relacionadas. Por exemplo, se uma palavra é o sujeito ou o predicado de uma sentença.
...
Entidades nomeadas são "objetos do mundo real" que possuem um nome. Por exemplo: uma pessoa, 
uma organização ou um país.
A propriedade doc.ents permite o acesso às entidades nomedas identificadas (previstas) pelo 
modelo de reconhecimento de entidades nomeadas
Ela retorna um iterável de objetos do tipo Span, possibilitando o acesso ao texto e ao 
marcador através do atributo .label_.
Neste caso, o modelo previu corretamente "Apple" como uma organização, "U.K." como uma 
entidade geopolítica.

"""

# Processar um texto
doc = nlp("A Apple está pensando em comprar uma startup do Reino Unido por $1 bilhão")
...
print("\n4")
"""
Uma dica: Para obter a definição dos marcadores mais comuns, você pode usar a função 
auxiliar spacy.explain.
Por exemplo, a sigla "GPE" para entidade geopolítica (geopolitical entity) não é muito 
intuitiva, mas o comando spacy.explain irá lhe explicar que se refere a países, cidades e 
estados.
